...
awk -F, '$(NF-1) != "559292" || $NF != "559292" {count++} END {print count}' SC_interaction.csv #4258
awk -F, '$(NF-1) == "559292" && $NF == "559292"' SC_interaction.csv > SC_interaction_filtered.csv
wc -l < SC_interaction_filtered.csv #853470
...
awk -F, '$NF != "NA" {count++} END {print count}' SC_interaction_filtered_with_scores.csv # 476916
awk -F, '$NF != "NA"' SC_interaction_filtered_with_scores.csv > SC_interaction_intersect.csv
wc -l < SC_interaction_intersect.csv # interaction information(in both database): 476916

# protein info
grep -c "^>" SC_pro_seq.fa # 6600
...
# protein not in interaction table: 833
